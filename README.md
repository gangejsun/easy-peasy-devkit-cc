...
> 🪟 **Windows 사용자라면 설치 전에 먼저 확인하세요** — 이 플러그인의 훅은
> `"bash <스크립트>"` 형태로 등록되어 있어, **Git for Windows**(Git Bash)가 없으면
> 에러 없이 조용히 전혀 작동하지 않습니다(WSL 사용자는 해당 없음). 비개발자도 따라 할
> 수 있는 5분 설치 가이드 → [`docs/windows-setup.md`](docs/windows-setup.md)
...
대화형으로 진행됩니다:
- **제품 형태 선택** — 웹 · 하이브리드 앱 · 네이티브 계열 · API 전용. 기술이 아니라 제품을 먼저 묻고, 이 답이 뒤 질문의 목록을 좁힙니다
...
| **규칙** | T0 41줄 + T1 9개 1,073줄 | T0는 훅이 상시 주입(플러그인 소유). T1은 `workflow-routing`이 **매 세션 상시**, 나머지는 경로 매칭 시 조건부 로드 |
...
| `security-check` | PreToolUse | 시크릿 하드코딩·파괴적 명령 차단 (exit 2) — Edit·Write·Bash 경로 전부 |
...
|----------|------|---------------------------|
| `epcc-planner` | Read, Grep, Glob, WebSearch, WebFetch | 불필요한 문서 양산 (Write 없음) |
...
각 단계 후 `doctor --default`로 확인하며 진행하고, 프로젝트 고유 자산은 보존합니다.
...
| 규칙 | generator가 `.claude/rules/`에 복사 (무조건 로드 3장 + 조건부 11장) | **T0 41줄 + T1 1,073줄, `install-rules.sh`로 설치 실증. 상시/조건부 구분 유지** |
...
- [그림 설명서](https://gangejsun.github.io/easy-peasy-devkit-cc/manual/) — 작업 지도·관리자 지도 ([English](https://gangejsun.github.io/easy-peasy-devkit-cc/manual/en/))
...
- [Windows 설치 가이드](docs/windows-setup.md) — Git for Windows·`jq` 준비물과 확인 체크리스트
