...
- 날짜: YYYY-MM-DD
- 장치: scripts/<훅>.sh 또는 skills/<스킬>   ← false-positive에만. 어느 장치가 잘못 걸렸는가
- 상황: 무엇을 하고 있었는가
- 실수: 무엇을 잘못했는가
- 교훈: 다음에 어떻게 다르게 할 것인가
```

`장치:` 줄이 있어야 `doctor --lessons`가 오탐을 **장치별**로 센다 — 카테고리 합계는 "무엇을
좁힐 것인가"를 말해주지 못한다. 없으면 본문의 첫 스크립트 이름으로 추정하고 (추정)이라 표시한다.
