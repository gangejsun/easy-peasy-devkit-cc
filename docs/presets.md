...
### 갱신이 설치된 프로젝트에 도달하게 한다

설치기는 스탬프 기반 멱등 갱신을 하지만 **아무도 다시 호출하지 않는다** — `epcc-init`과
`epcc-migrate`에서만 불린다. 그래서 플러그인을 올려도 오래 전 설치한 프로젝트는 옛 사본을
그대로 쓴다. 유지자가 부지런히 감사해도 그 결과가 도달하지 않으면 감사하지 않은 것과 같다.

`session-brief` 훅이 매 세션 대조한다 — `assembly.json`의 `packVersion`과 플러그인 버전,
`.claude/rules/`의 `epcc-rule-version`과 플러그인 원본. **어긋날 때만** 재실행을 안내한다.
