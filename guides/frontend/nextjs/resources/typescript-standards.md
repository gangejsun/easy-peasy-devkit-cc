...
strict 모드 전제의 타입 작성 표준: any 금지, 타입 가드, 유틸리티 타입, null 처리, 네이밍.
...
// 데이터 접근 자체는 백엔드 축의 클라이언트가 담당한다 — 이 계층은 타입 경계만 소유한다
export async function getTaskById(id: string): Promise<Task | null> {
  const rows = await getTasks({ id })     // 구현은 backend-guide의 데이터 액세스 절
  return rows[0] ?? null
}
