---
name: backend-guide
description: Express 5 + Prisma 6 + PostgreSQL 백엔드 개발 가이드. 라우터와 응답 봉투, Zod 요청 검증, 소유권 쿼리, 커서 페이지네이션, 베어러 토큰 인증과 재발급, CORS, 에러 코드 매핑, 테스트를 다룬다. Use when creating or modifying Express routers, request validation, Prisma queries, authentication middleware, error handling, CORS, pagination, or backend tests.
---
...
2. `src/db/tasks.ts`에 쿼리 함수를 넣는다 — 첫 인자가 `ownerId`, 소유권은 `where`에 건다
3. 변이면 영향 행 수를 본다: `count === 0`이면 `AppError('NOT_FOUND')`
4. `src/http/routers/tasks.ts`에 핸들러를 붙이고 `parseBody`/`parseQuery`의 **반환값**을 쓴다
5. 성공 봉투만 만든다 — 단건 `{ data }` · 목록 `{ data, nextCursor }` · 생성 201 · 삭제 204
6. 실패는 던진다. 라우터에서 `res.status(...).json(...)`을 쓰지 않는다
7. 소유자와 타인 두 주체로 200과 404를 **짝으로** 단언한다 (`resources/complete-example.md`)

### 새 보호 경로를 추가한다

1. 라우터를 만들고 **첫 줄에** `router.use(requireAuth)`를 건다 — 라우트마다 걸지 않는다
2. 역할이 필요한 라우트에만 `requireRole('admin')`을 덧붙인다
3. 주체는 `req.user`에서만 읽는다 — 본문·쿼리·헤더의 소유자 값은 공격자가 고른다
4. 소유권 실패는 404다. 403은 역할 부족에만 쓴다
5. `makeApp()`에 마운트 줄을 넣는다 — `notFound`·`errorHandler`보다 **앞**이다
6. 새 오리진이 생기면 `WEB_ORIGINS` 환경변수에 넣는다 — 코드에 적지 않는다
7. 토큰 없이 401, 토큰을 넣으면 200이 나오는지 **짝으로** 확인한다
...
| 하려는 일 | 읽을 파일 |
| --- | --- |
| 계층 경계(routes → services → db) · Express 5 앱 조립 · 라우터 마운트 · 404/에러 미들웨어 순서 · 비동기 핸들러 | `resources/project-structure.md` |
| Prisma Client 싱글턴 · 소유권 필터 · 영향 행 수 확인 · 트랜잭션 · 페이지네이션 · N+1과 select · 마이그레이션 실행 | `resources/data-access.md` |
| Zod 스키마 작성 · 요청 파싱 헬퍼 · DB 제약 이중화 · 환경변수 스키마 · 부팅 시점 실패 | `resources/input-validation.md` |
| 도메인 에러 → 상태 코드 매핑 · Prisma 에러 코드 판별(P2002·P2025) · ZodError 정규화 · 존재 누설 금지 | `resources/error-handling.md` |
| Vitest 설정 · supertest 라우터 테스트 · 실 DB 통합 테스트 · 테스트 DB 격리 · 시드와 정리 | `resources/testing.md` |
| graceful shutdown · 헬스체크(live/ready) · 커넥션 풀 사이징 · 구조적 로깅 · 마이그레이션 배포 순서 · 느린 쿼리 관측 | `resources/operations.md` |
| 라우터 추가 · 요청/응답 봉투 · 커서 페이지네이션 응답 · 상태 코드 · CORS · 검증 헬퍼 소비 | `resources/api-endpoints.md` |
| 토큰 검증 · `requireAuth` 배선 · `req.user` 타입 확장 · 역할/스코프 · 로그인·로그아웃 · 401 재발급 계약 | `resources/auth-boundaries.md` |
| 마이그레이션 → 스키마 → 쿼리 → 서비스 → 라우터 → 테스트 전 구간 | `resources/complete-example.md` |
