...
export async function createTask(
  ...
  if (error) {
    if (error.code === '23505') return { ok: false, formError: 'This task already exists' }
...
Both are equally public: **arguments are client input**, so a directly-called `deleteTask(id)`
