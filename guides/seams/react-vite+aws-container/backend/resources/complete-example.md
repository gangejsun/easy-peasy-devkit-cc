...
# 완전 예제 — `tasks` 리소스 한 바퀴

마이그레이션 → 스키마 → 쿼리 → 서비스 → 핸들러 → 테스트를 **하나의 도메인**으로 관통한다.
새 리소스는 이 순서를 그대로 따른다. 목표: 로그인한 사용자가 자신의 작업을 목록
...
export const tasks = pgTable('tasks', {
  ...
  index('tasks_owner_created_idx').on(t.ownerId, t.createdAt, t.id),  // 정렬 키 전체를 덮는다
  uniqueIndex('tasks_owner_title_key').on(t.ownerId, t.title),
])
...
npx drizzle-kit generate      # → drizzle/0001_wild_tasks.sql
```

```sql
-- drizzle/0001_wild_tasks.sql  (생성물 — 읽고 검토한다. 손으로 먼저 쓰지 않는다)
CREATE TABLE "tasks" (
...
CREATE INDEX "tasks_owner_created_idx" ON "tasks" USING btree ("owner_id","created_at","id");
CREATE UNIQUE INDEX "tasks_owner_title_key" ON "tasks" USING btree ("owner_id","title");
```
...
정본 정의는 `resources/input-validation.md`의 `src/schemas/tasks.ts`다. 이 예제가 쓰는 형태:
...
const TaskFields = z.strictObject({              // 기본값 없는 정의가 원본이다
  title: z.string().trim().min(1).max(120),
  body: z.string().max(20_000),
})
export const CreateTaskInput = TaskFields.extend({ body: TaskFields.shape.body.default('') })
export const UpdateTaskInput = TaskFields.partial().refine(     // ← Create.partial()이 아니다
...
`CreateTaskInput.partial()`로 만들면 `.default('')`가 살아남아 `{ title: 'x' }` PATCH가
...
export async function insertTask(ownerId: string, input: CreateTaskInput) {
  // ownerId를 뒤에 둔다 — 앞에 두면 input의 동명 키가 덮어쓴다
  const [row] = await db.insert(tasks).values({ ...input, ownerId }).returning()
  return row
}
export async function findOwnedTask(ownerId: string, id: string) {
...
export async function listOwnedTasks(
  ...
  return db.select().from(tasks)
    .where(and(
      ...
      cursor ? sql`(${tasks.createdAt}, ${tasks.id}) < (${cursor.createdAt}, ${cursor.id})`
             : undefined,
      archived ? isNotNull(tasks.archivedAt) : isNull(tasks.archivedAt),   // 미지정 = 활성만
    ))
    .orderBy(desc(tasks.createdAt), desc(tasks.id))
    .limit(limit + 1)                       // 다음 페이지 유무 판단용 1행
}
// patch 타입을 Partial<NewTask>로 열면 patch.ownerId가 실려 소유권이 이전된다
export async function updateOwnedTask(ownerId: string, id: string, patch: UpdateTaskInput) {
  const [row] = await db.update(tasks).set({ ...patch, updatedAt: new Date() })
    .where(and(eq(tasks.id, id), eq(tasks.ownerId, ownerId))).returning()
  return row ?? null                        // null = 없거나 내 것이 아니다
}
export async function deleteOwnedTask(ownerId: string, id: string) {
  const rows = await db.delete(tasks)
    .where(and(eq(tasks.id, id), eq(tasks.ownerId, ownerId))).returning({ id: tasks.id })
...
import * as q from '@/db/queries/tasks'
import type { CreateTaskInput, ListTasksQuery, UpdateTaskInput } from '@/schemas/tasks'
...
export async function listTasks(ownerId: string, query: ListTasksQuery) {
  const rows = await q.listOwnedTasks(
...
export async function createTask(ownerId: string, input: CreateTaskInput) {
  try {
    return await q.insertTask(ownerId, input)
  } catch (err) {
...
tasksRoutes.get('/', async (c) => {
  const page = await tasks.listTasks(c.get('user').sub, queryParams(c, ListTasksQuery))
  return c.json({ data: page.items, nextCursor: page.nextCursor })
})
tasksRoutes.post('/', async (c) => {
  const created = await tasks.createTask(c.get('user').sub, await jsonBody(c, CreateTaskInput))
  return c.json({ data: created }, 201)
})
tasksRoutes.get('/:id', async (c) => {
  const { id } = pathParam(c, TaskIdParam)
  return c.json({ data: await tasks.getTask(c.get('user').sub, id) })
})
tasksRoutes.patch('/:id', async (c) => {
  const { id } = pathParam(c, TaskIdParam)
  const patch = await jsonBody(c, UpdateTaskInput)
  return c.json({ data: await tasks.updateTask(c.get('user').sub, id, patch) })
})
tasksRoutes.delete('/:id', async (c) => {
  const { id } = pathParam(c, TaskIdParam)
  await tasks.deleteTask(c.get('user').sub, id)
  return c.body(null, 204)
})
```

`src/app.ts`에서 `app.use('/api/*', requireAuth)` **다음 줄에**
`app.route('/api/tasks', tasksRoutes)`로 마운트한다 (공개 라우트인 `/api/auth`만 그 앞이다).
...
```ts
// tests/integration/tasks.test.ts  (3계층: 실 DB)
import { describe, expect, it } from 'vitest'
import { createTask, getTask, updateTask, deleteTask } from '@/services/tasks'

const ALICE = '11111111-1111-1111-1111-111111111111'
const BOB = '22222222-2222-2222-2222-222222222222'

describe('tasks 서비스', () => {
  it('생성한 작업을 소유자만 읽는다', async () => {
    const created = await createTask(ALICE, { title: '회의록', body: '' })
    await expect(getTask(ALICE, created.id)).resolves.toMatchObject({ title: '회의록' })
    await expect(getTask(BOB, created.id)).rejects.toMatchObject({ status: 404 })
  })
  it('남의 작업 수정·삭제는 404다 (0행을 성공으로 응답하지 않는다)', async () => {
    const created = await createTask(ALICE, { title: '개인 메모', body: '' })
    await expect(updateTask(BOB, created.id, { title: 'x' })).rejects.toMatchObject({ status: 404 })
    await expect(deleteTask(BOB, created.id)).rejects.toMatchObject({ status: 404 })
    await expect(getTask(ALICE, created.id)).resolves.toBeDefined()   // 그대로 남아 있다
  })
  it('제목만 바꾸는 PATCH가 본문을 지우지 않는다', async () => {
    const created = await createTask(ALICE, { title: '초안', body: '지켜야 할 본문' })
    const updated = await updateTask(ALICE, created.id, { title: '수정본' })
    expect(updated.body).toBe('지켜야 할 본문')     // Create.partial()이면 여기서 ''가 된다
  })
  it('같은 소유자의 제목 중복은 409다', async () => {
    await createTask(ALICE, { title: '중복', body: '' })
    await expect(createTask(ALICE, { title: '중복', body: '' }))
