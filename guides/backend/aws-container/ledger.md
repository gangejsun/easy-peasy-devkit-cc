...
| `ObjectStorage` | portability-boundaries.md | `interface ObjectStorage { put(key: string, body: Uint8Array, contentType: string): Promise<void>; presignGet(key: string, expiresInSec: number): Promise<string> }` — `expiresInSec`는 **초** 단위다. 앱 코드가 import하는 유일한 저장소 타입이고 AWS SDK를 아는 것은 구현(`src/storage/s3.ts`)뿐. 저장 키는 서버가 만든다(`tasks/<ownerId>/<uuid>`) | 저장소 포트 (앱이 보는 유일한 타입) |
