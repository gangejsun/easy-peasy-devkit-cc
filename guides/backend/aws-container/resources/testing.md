...
**`UpdateTaskInput.safeParse({})`가 실패하는지**(빈 PATCH 거절 = 남은 기본값 없음).
...
```ts
// tests/handler/tasks.post.test.ts
import { describe, expect, it, vi } from 'vitest'

vi.mock('@/db/queries/tasks', () => ({
  insertTask: vi.fn(async (ownerId: string, input: { title: string }) =>
...
const post = (body: unknown) => app.request('/api/tasks', {
  method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body),
})

describe('POST /api/tasks', () => {
  it('검증을 통과하면 201과 data 봉투를 반환한다', async () => {
    const res = await post({ title: '첫 작업' })
    expect(res.status).toBe(201)
    expect((await res.json()).data.title).toBe('첫 작업')
  })
...
```ts
// tests/integration/tasks.ownership.test.ts
import { describe, expect, it } from 'vitest'
import { insertTask, findOwnedTask, updateOwnedTask, deleteOwnedTask } from '@/db/queries/tasks'

const ALICE = '11111111-1111-1111-1111-111111111111'
const BOB = '22222222-2222-2222-2222-222222222222'
const task = (title: string) => ({ title, body: '' })      // insertTask는 파싱된 입력을 받는다

describe('소유권 격리', () => {
  it('다른 사용자의 작업은 조회되지 않는다', async () => {
    const row = await insertTask(ALICE, task('alice의 작업'))
    expect(await findOwnedTask(BOB, row.id)).toBeNull()
  })
  it('다른 사용자의 작업 수정은 0행이다', async () => {
    const row = await insertTask(ALICE, task('alice의 작업'))
    expect(await updateOwnedTask(BOB, row.id, { title: 'hijacked' })).toBeNull()
    expect((await findOwnedTask(ALICE, row.id))!.title).toBe('alice의 작업')
  })
  it('다른 사용자의 작업 삭제는 0행이다', async () => {
    const row = await insertTask(ALICE, task('alice의 작업'))
    expect(await deleteOwnedTask(BOB, row.id)).toBe(0)
  })
  it('같은 사용자의 제목 중복은 23505로 거절된다', async () => {
    await insertTask(ALICE, task('중복'))
    await expect(insertTask(ALICE, task('중복'))).rejects.toMatchObject({ code: '23505' })
    await expect(insertTask(BOB, task('중복'))).resolves.toBeDefined()   // 사용자별 유일
  })
})
```
