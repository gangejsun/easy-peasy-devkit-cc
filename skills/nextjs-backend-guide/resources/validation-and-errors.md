...
- The **same schema** is imported by the Route Handler, the Server Action, and tests —
  never duplicate a schema inline in a handler.
...
- Unknown keys are stripped by default; use `z.strictObject({...})` when an unknown
  key should be a 400 instead.

## Query-param schemas

Query params arrive as strings — coerce them, and build the input object with
`Object.fromEntries(request.nextUrl.searchParams)` so absent keys stay `undefined`
and defaults apply:
...
Without the preprocess, an empty optional field fails as `""` (too short) instead of
being treated as absent — a classic form-only bug that never shows up in JSON tests.
...
Every Zod rule that protects data integrity is **duplicated as a Postgres constraint**.
Zod guards the request path; constraints guard every other path (RPC, admin client,
SQL console, future bugs). One without the other is half a defense.
...
```sql
-- The DDL twin of createNoteSchema (structure per the T1 data-modeling card)
...
"Never touch `process.env` directly" is enforced by one Zod-validated module.
A missing or malformed variable fails **at boot**, not at first use in production:
...
const envSchema = z.object({
  ...
  SUPABASE_SERVICE_ROLE_KEY: z.string().min(1), // server-only — never NEXT_PUBLIC_*
})

// .parse (not safeParse) is correct here: a bad env is a deploy error, crash loudly
export const env = envSchema.parse(process.env)
```

Server code imports `env` instead of reading `process.env` — typos become compile
errors and every required variable is declared in one place.
...
| `PGRST116` | `.single()` found 0 rows (missing **or RLS-hidden**) | 404 | `not_found` |
...
- Validation failure → `fieldErrors` (rendered next to inputs by `useActionState`).
- Auth/DB/unknown failure → `formError` with a generic message; log the real error.
- `redirect()` is the one intentional throw — keep it outside `try/catch`.

## Logging rules

- Log the full Supabase/Postgres error object server-side (`console.error` or your
  logger) with a `[handler-name]` prefix for traceability.
