...
| 작업 유형 | 판별 조건 | 실행 Step |
|----------|----------|----------|
| 디자인 시스템 생성 | 새 프로젝트, 전체 스타일 가이드, 디자인 리뉴얼 | Step 2 → 2.5 → **2.6 → 2.7** → 3 → 4 → 5 |
| 페이지별 디자인 | 특정 페이지/화면 디자인 | Step 2(페이지 모드) → 2.5 → **2.6 → 2.7** → 3 → 5 |
| 도메인 검색 | 특정 영역(색상, 타이포, 스타일 등) 정보만 필요 | Step 3 |
| 스택 가이드라인 | 프레임워크별 구현 가이드 필요 | Step 4 |

요구사항에서 추출할 정보 — `dev/docs/service/service-plan-<name>.md`가 있으면
**화면 목록과 사용자 플로우를 거기서 읽는다**(P0-D 산출물). 없으면 사용자 요청에서 뽑는다:
...
### Step 2.6: 시안 캔버스 — 기본 호출

Step 2.5를 통과했고 **신규·재설계 화면이 1개 이상이면 캔버스를 만든다.** 선택이 아니라
기본값이다 — 코드 이전에 방향 합의를 끝내는 자리이고, 그것을 P4로 미루면 가장 비싼 곳에서
논쟁이 일어난다.

**건너뛰는 경우는 둘뿐이다**: ⓐ `/design`이 없거나 저장이 비활성 — **판정 불가이지 결함이
아니다**, 캔버스 없이 Step 4로 간다 ⓑ 사용자가 명시적으로 거절. 둘 중 어느 쪽도 구현을
막지 않는다.

**Step 2.5 이후에만 호출한다.** `/design`은 `.claude/rules/ui-design.md` §1의 기본값 3군집
캘리브레이션을 모른다 — 브리프만 주면 심사가 막으려던 크림+세리프·근검정+산성악센트·괘선
다단이 그대로 나온다. 토큰을 **결정된 값으로 고정해서** 넘긴다:

```
/design <프로젝트> 화면 N개 — <화면1>·<화면2>·<화면3>

토큰 고정 (새로 만들지 말 것) — MASTER.md에서 옮긴다:
- 색: <이름 붙인 hex 4~6개>
- 타입: 디스플레이 <폰트> / 본문 <폰트>
- 레이아웃: <1패스의 한 문장>
- 시그니처: <§2에서 지명한 단 하나> — 여기에만 대담함을 쓰고 나머지는 조용하게
규격: <390x844 등>
실제 문안 사용(lorem 금지). 빈·로딩·에러 상태도 아트보드로 포함.
```

마지막 줄이 Step 6의 「상태 3종」을 **구현 전으로 당긴다** — 지금은 그 셋이 렌더 증명에서야
발견된다.

사용자가 캔버스에서 직접 고치고 Save한다. **손으로 고친 그것이 최종 디자인 결정이다.**

### Step 2.7: 되쓰기 + 대조

캔버스 확정 직후에 한다. 캔버스에만 남은 결정은 grep도 diff도 되지 않아 다음 세션에
존재하지 않는다.

1. 발행된 Artifact를 읽어 로컬 HTML로 받는다.
2. `dev/docs/design/<project>/MASTER.md`의 네 자리를 갱신한다:

| 갱신 대상 | 무엇을 |
|----------|--------|
| `### Color Palette` · `### Typography` | 캔버스에서 바뀐 hex·폰트 |
| `## Signature` | 시그니처가 바뀌었으면 새 지명 |
| `## 방향 심사` | 무엇을 왜 바꿨는지 한 줄 |
| `## 캔버스` **(없으면 새로 만든다)** | 캔버스 URL · 확정 일자 · 아트보드 목록 |

`## 캔버스` 절이 **다음 세션이 화면 구성을 찾아가는 유일한 경로**다. 이 절이 없으면
캔버스는 없는 것으로 판정된다(아래 스크립트가 그렇게 읽는다).

3. 대조한다 — **exit 0이 이 Step의 종료 술어다**:

```bash
bash ${CLAUDE_SKILL_DIR}/scripts/design-sync.sh --master dev/docs/design/<project>/MASTER.md --canvas <내려받은.html>
```

| 종료 코드 | 뜻 | 행동 |
|----------|-----|------|
| 0 | 색·폰트 일치 | 시그니처만 육안 대조하고 다음 Step |
| 1 | 불일치 | **되쓰기가 덜 끝났다.** diff를 사용자에게 보고하고 판단을 받는다 — 차단하지 않는다 |
| 2 | 판정 불가 | 「캔버스 미사용」이면 그대로 진행 · 「선언은 있는데 도달 못 함」이면 그 사실을 보고한다 |

색·폰트는 스크립트가 보고 **시그니처는 사람이 본다** — 산문이라 집합 연산이 불가능하다.
실측 가능한 것을 눈으로 확인했다고 적지 않는다.

**Step 2.6을 건너뛴 경우 Step 2.7도 건너뛴다** — 되쓸 것이 없다. 대신 방향 심사 1패스의
레이아웃 항목을 화면 수만큼 남긴 채로 Step 4로 간다.
...
구현은 **두 문서를 읽는다** — 관할이 갈려 있어서 하나만 보면 나머지를 잃는다:

| 읽는 것 | 어디서 |
|---------|--------|
| 색·폰트·spacing·shadow·시그니처·안티패턴 | `MASTER.md` |
| 요소 배치·계층·실제 카피·빈/로딩/에러의 생김새 | **캔버스** (`## 캔버스`가 가리킨다) |

`## 캔버스` 절이 없으면 캔버스는 없는 것이고, `MASTER.md`(+`pages/`)가 구성까지 포함한
유일한 정본이다 — 캔버스를 찾아 헤매지 않는다.

스택 관행(컴포넌트 경계·클래스 병합·파일 배치)의 정본은 프로젝트의 `frontend-guide`
스킬이다. 예(Next.js 조합):
...
- 토큰 대조: [scripts/design-sync.sh](scripts/design-sync.sh) (`--help` 로 사용법)
