...
---
<!-- epcc-guide-baseline: verified 2026-08-22 hono@4 @hono/node-server@1 typescript@5 drizzle-orm@0 drizzle-kit@0 pg@8 jose@5 zod@4 vitest@2 postgres@16 aws-cdk-lib@2 -->
...
```ts
// ✅ 파싱된 값만 서비스로 넘어간다
const body = await jsonBody(c, CreateNoteInput)   // 실패 시 422 봉투로 throw
const note = await createNote(user.sub, body)

// ❌ 캐스팅은 검증이 아니다 — 런타임에 아무것도 막지 못한다
const body = (await c.req.json()) as CreateNoteInput
await createNote(user.sub, body)   // title: 12MB 문자열도 그대로 통과
```

`z.infer<typeof CreateNoteInput>`을 서비스 시그니처에 쓰면 스키마가 곧 계약이 된다.
...
export function findOwnedNote(ownerId: string, id: string) {
...
const [updated] = await db.update(notes).set({ title, updatedAt: new Date() })
