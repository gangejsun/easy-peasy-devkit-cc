...
`/api/auth/*`는 인증을 만나지 않고, 뒤에 등록된 `/api/notes`는 반드시 만난다. `requireAuth`를
...
notesRoutes.get('/', async (c) => {
  const q = queryParams(c, ListNotesQuery)
  const page = await notes.listNotes(c.get('user').sub, q)
  return c.json({ data: page.items, nextCursor: page.nextCursor })
})

notesRoutes.post('/', async (c) => {
  const body = await jsonBody(c, CreateNoteInput)
  const created = await notes.createNote(c.get('user').sub, body)
  return c.json({ data: created }, 201)
})

notesRoutes.patch('/:id', async (c) => {
  const { id } = pathParam(c, NoteIdParam)
  const body = await jsonBody(c, UpdateNoteInput)
  return c.json({ data: await notes.updateNote(c.get('user').sub, id, body) })
})

notesRoutes.delete('/:id', async (c) => {
  const { id } = pathParam(c, NoteIdParam)
  await notes.deleteNote(c.get('user').sub, id)     // 0행이면 서비스가 404를 던진다
...
- 요청 스키마 `ListNotesQuery`(`limit`·`cursor`·`archived`)의 정본 정의는
...
// 서비스에서
try {
  return await insertNote(ownerId, input)
} catch (err) {
