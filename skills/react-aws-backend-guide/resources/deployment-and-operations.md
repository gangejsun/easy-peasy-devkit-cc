...
curl -s -o /dev/null -w '%{http_code}\n' localhost:8080/api/notes    # 401 (토큰 없음)
curl -s -X POST localhost:8080/api/notes -H "Authorization: Bearer $TOKEN" \
  -H 'Content-Type: application/json' -d '{"title":"첫 노트"}' | jq
```
...
- 커서 페이지네이션이 인덱스를 타는지 `EXPLAIN`으로 확인한다. `Seq Scan on notes`가 보이면
