...
아래 `src/schemas/notes.ts`가 **요청 스키마의 정본**이다. 다른 리소스 파일에 나오는
`CreateNoteInput` / `UpdateNoteInput` / `ListNotesQuery` 인용은 전부 이 정의를 가리킨다.
...
export const UpdateNoteInput = NoteFields.partial().refine(
...
- **PATCH 스키마를 생성 스키마에서 `.partial()`로 뽑지 않는다.** `.partial()`은 `.default()`를
  지우지 못한다 — `CreateNoteInput.partial().parse({})`는 `{ body: '' }`를 내놓는다. 그러면
  ① "필드 하나 이상" `refine`이 영구 통과해 빈 PATCH가 거절되지 않고 ② `set({ ...patch })`가
  **건드리지도 않은 본문을 빈 문자열로 덮어쓴다.** 기본값 없는 `NoteFields`가 원본이어야 한다
...
- 선언한 쿼리 파라미터는 **반드시 쿼리 함수까지 배선한다**. `archived`는
  `listOwnedNotes(ownerId, limit, cursor, archived)`가 소비한다 (`resources/data-access.md`) —
...
| 사용자별 제목 중복 금지 | 사전 조회는 경합에 진다 | `uniqueIndex('notes_owner_title_key')` |
