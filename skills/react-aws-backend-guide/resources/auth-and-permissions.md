...
export async function updateNote(ownerId: string, id: string, patch: UpdateNoteInput) {
  const updated = await updateOwnedNote(ownerId, id, patch)   // 소유권이 WHERE에 있다
...
if (!note) throw new AppError(404, 'NOT_FOUND', '...')
if (note.ownerId !== ownerId) throw new AppError(403, 'FORBIDDEN', '...')
```
...
```ts
// src/db/queries/admin-notes.ts — 소유자 조건이 없는 유일한 파일 (파일 단위로 격리한다)
export async function findAnyNote(id: string) {
...
adminRoutes.get('/notes/:id', async (c) => {
  const note = await findAnyNote(pathParam(c, NoteIdParam).id)
  if (!note) throw new AppError(404, 'NOT_FOUND', 'Note not found')   // 부재도 미인가도 404
  return c.json({ data: note })
})
```

`grep -rn "admin-notes" src/`가 곧 감사 목록이다 — `queries/notes.ts`에는 소유자 인자 없는 함수가 **하나도 없어야 한다.**
