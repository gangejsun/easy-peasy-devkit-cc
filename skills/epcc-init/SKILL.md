...
### Step 9.5: 스택 가이드 확보 (질문 없음 — 시스템이 판단한다)

확정된 조합(`techStack.presets` + 백엔드 차원)으로 아래 표를 조회해 **묻지 않고 실행한다.**
사전 제작본을 쓸지 생성할지는 구현 세부이지 사용자가 답할 질문이 아니다.
...
#### 사전 제작본 경로 — 기준선 점검 (묻지 않고 점검한다)

사전 제작본 SKILL.md 첫머리의 `<!-- epcc-guide-baseline: ... -->` 줄과 프로젝트의 실제
의존성(`package.json`)을 **메이저 버전 기준으로** 대조한다.
...
승인을 묻지 않고 `stack-guide-generator`를 호출한다. 진행 상황만 알린다:

```
확정된 조합: <프론트엔드 프리셋> × <백엔드 프리셋> (+ 변경된 차원)
이 조합의 frontend-guide / backend-guide를 생성합니다 — 몇 분 걸립니다.
(규격서 기반 생성 → 기계 검증 → 독립 감사 → 수리 → 재검증)
```

- config가 채워졌으므로 그 스킬의 Step 1 인터뷰는 생략되지만, **Step 3~6 검증 루프는
  생략하지 않는다** — 실측에서 매 라운드 자기보고 통과 산출물에 실결함이 남아 있었다
- 호출 시 **두 축의 프리셋 `notes`를 함께 전달한다.** 가이드는 한 축의 함수가 아니라
  ...
  두 가이드의 내용을 함께 결정한다
- 생성이 실패하거나 검증 게이트를 통과하지 못하면 **설치하지 않고** 실패 항목을 보고한다.
  이때만 사용자 판단이 필요하다 (부분 산출물을 남길지, 나중에 재시도할지)

#### 사전 제작본을 늘리는 기준

조합은 27개이고 전부 사전 제작하면 가이드 45개(약 9만 줄)가 된다 — 유지 불가능하며,
감사받지 않는 사전 제작본은 부패해서 **없는 가이드보다 나쁘다**(틀린 지침을 신뢰하게 만든다).
그래서 사전 제작본 추가는 개수가 아니라 조건으로 정한다:

> **정기 감사 대상으로 등록할 수 있을 때만 사전 제작한다.** 아무도 감사하지 않을 조합은
> 신선 생성이 엄격히 우월하다 — 생성본은 그 시점의 최신 지식으로 태어나기 때문이다.
