...
규범과 범위는 `gemini-claude-loop`의 Step 0.5와 **동일하다** — 두 외부 루프가 같은 규범을
다르게 주장하지 않기 위해서다 (평가 v3 · E-05).
...
- 자동 경로(epcc-reviewer의 cross-check)에서는 **동의 없이 실행하지 않는다**
- 시크릿·자격증명이 diff에 포함되면 중단한다
