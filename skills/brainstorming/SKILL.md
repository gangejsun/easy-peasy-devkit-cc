...
| 호출 상황 | 전환 대상 |
|----------|----------|
| 기획 파이프라인 진행 중 (epcc-planner 경유) | epcc-planner에게 설계 요약을 반환 → research 진행 |
| 단독 호출 — 진입 조건 미충족 2개 이상 | 프롬프트 강화 → PRD 생성 → 개발 문서 생성 흐름 진행 |
| 단독 호출 — 진입 조건 미충족 1개 이하 | 프롬프트 강화 → 구현 흐름 진행 |
| 단독 호출 — 기존 기능 확장 | 개발 문서 생성으로 전환 |
...
epcc-planner 경유 시, 설계 결과를 **구조화된 요약**으로 반환.
