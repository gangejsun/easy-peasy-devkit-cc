...
4-2. 핵심 화면 목록
4-3. 화면 간 네비게이션 플로우
4-4. 화면 데이터 요구 — 화면마다 **읽는 필드 · 쓰는 필드 · 목록 여부 · 빈/로딩/에러 분기**를
     표로 적는다 (§6 「화면 데이터 요구」). 와이어 계약(`dev/docs/api/wire-contract.md`)과
     데이터 모델 초안은 이 표에서 도출된다 — 없으면 계약이 화면과 무관하게 지어진다
4-5. 데이터 모델 초안 — 4-4의 필드를 엔티티로 묶는다 (논리 수준. 물리 타입·마이그레이션은
     구현 단계의 일이다)
...
- FR 도출 시 UI 화면 1:1 매핑 함정 회피 — 4-4의 화면 데이터 요구 표는 IA 산출물(화면이
  어떤 데이터에 닿는가)이고 FR(사용자가 무엇을 할 수 있는가)이 아니다. 둘을 섞지 않는다
